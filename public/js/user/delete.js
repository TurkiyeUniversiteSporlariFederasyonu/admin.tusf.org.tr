window.addEventListener('load', () => {
  document.addEventListener('click', event => {
    if (ancestorWithClassName(event.target, 'restore-user-button')) {
      const target = ancestorWithClassName(event.target, 'restore-user-button');
      ...
      createConfirm({
        ...
        serverRequest('/user/restore?id=' + id, 'POST', {}, res => {
          if (!res.success)
