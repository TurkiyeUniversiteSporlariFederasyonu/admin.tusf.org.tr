...
module.exports = (data, callback) => {
  ...
  if (data.template == 'manager_password_reset') {
    ...
    fetch(`https://api.elasticemail.com/v2/email/send?apiKey=${ELASTIC_EMAIL_API_KEY}&isTransactional=true&template=manager_password_reset&merge_id=${data._id.trim()}&merge_email=${data.email.trim()}&merge_name=${data.name.trim()}&merge_token=${data.token.trim()}&to=${data.to.trim()}&charset=utf-8`, {
      method: 'POST'
    })
      .then(data => data.json())
